...
/** Remove <p> / </p> / <br> tags for list excerpts; keep inline text; then shorten */
function excerpt_text(string $html, int $len): string {
  $s = preg_replace('/<\/?p[^>]*>|<br\s*\/?>/i', ' ', (string)$html); // drop p/br → spaces
  $s = strip_tags($s);                                                // remove any other tags
  $s = preg_replace('/\s+/', ' ', trim($s));                          // collapse whitespace
  return short_text($s, $len);
}

/* Slugify helper (convert title → URL-friendly text) */
...
function get_content(PDO $pdo, string $key, string $default=''): string {
  // ensure table exists (harmless if already created)
  $pdo->exec("
...
/* ----- fetch blogs from DB ----- */
if ($hasQ) {
  // Featured = newest match by title
...
<main class="page">
  <div class="wrap">
    ...
    <?php else: ?>
      ...
      <!-- All Blog Posts (hide heading if empty) -->
      <?php if ($blogs): ?>
...
<script>
// Web Share API + fallback; builds absolute URL from relative link
