...
// (optional but handy during dev)
if ($pdo instanceof PDO) {
  $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
}

/* ---------- helpers ---------- */
// Safe HTML escape (short & consistent)
if (!function_exists('h')) {
  function h($v){ return htmlspecialchars((string)$v, ENT_QUOTES, 'UTF-8'); }
}

/* If your site lives in a subfolder like /paramount, set it; otherwise '' */
$BASE_URL = '';

function img_url(string $path, string $BASE_URL=''): string {
  $path = trim($path);
  if ($path === '') return '';
  if (preg_match('~^(https?:)?//|^data:~i', $path)) return $path; // absolute or data:
  if ($path[0] === '/') return $path;                               // already root-relative
  return rtrim($BASE_URL, '/') . '/' . ltrim($path, '/');           // make root-relative
}
...
<script>
(function () {
  // Exclude hero-inner content from animations
  const excludeHero = (el) => el.closest('.hero-inner');

  // Targets across the page (no wrapper needed)
  ...
  // Seed .reveal (skip hero content)
  targets.forEach(el => {
    if (excludeHero(el)) return;
