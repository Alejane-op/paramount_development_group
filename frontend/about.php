<?php 
  // highlight About tab in your navbar
  $active = 'about.php';

  // connect DB (must provide $pdo = PDO in this file)
  ...
  // key-value getter that matches adminAbout.php keys
  ...
  /* --------------- fetch all content --------------- */
  $heroTitle = cget($pdo, 'hero_title', 'Our Paramount<br>Development Background');  // allow <br>
...
<body>
  ...
  <main class="about-wrap">
    ...
    <section class="hero-about">
      ...
      <div class="hero-content-about">
        <div class="hero-copy-about">
          <!-- hero title allows HTML (e.g., <br>) -->
          <h1 style="color:#fff !important;"><?= $heroTitle ?></h1>
          <?php if (trim($heroPara)!==''): ?>
            <p class="muted"><?= $heroPara ?></p> <!-- raw: let admin format -->
          <?php endif; ?>
        </div>
      </div>
    </section>
    ...
    <!-- ========== OUR VISION ========== -->
    <section class="section" style="margin-top: -145px;">
      <div class="container">
        <h2>Our Vision</h2>
        <?php if (trim($vision)!==''): ?>
          <p class="vision muted mt-12"><?= $vision ?></p> <!-- raw for rich text -->
        <?php else: ?>
          ...
          <?php else: ?>
            <?php foreach ($values as $v): ?>
              <div class="value-item">
                <!-- icon class saved in DB, e.g., "fa-solid fa-user-shield" -->
                <i class="<?= h($v['icon']) ?>"></i>
                <h3><?= h($v['title']) ?></h3>
                <p><?= nl2br(h($v['body'])) ?></p>
              </div>
    ...
    <!-- ========== OUR STRATEGY ========== -->
    <section class="section strategy" style="margin-top: -100px;">
      <div class="container">
        ...
        <!-- ========== OUR COMMITMENT ========== -->
        <h2 class="h2 mt-64">Our Commitment</h2>
        <?php if (trim($commitment)!==''): ?>
          <p class="commitment"><?= $commitment ?></p> <!-- raw for rich text -->
        <?php else: ?>
    ...
    <!-- ========== OUR TEAM (CTA) ========== -->
    <section class="section team" style="margin-top: -90px; margin-bottom: -30px;">
      <div class="container">
  ...
  <!-- Font Awesome (for core values icons) -->
...
(function () {
  ...
  // Targets to animate (exclude hero-content-about)
  const targets = scope.querySelectorAll([
    // headings & paragraphs across sections
  ...
  targets.forEach(el => {
    if (el.closest('.hero-content-about')) return; // EXCLUDE hero content
    el.classList.add('reveal');
  });

  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      // add 'in' when 25% visible, remove when out → gives fade-in/out
