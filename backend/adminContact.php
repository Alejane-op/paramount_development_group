<?php
// backend/adminContact.php — contact page settings + read-only inbox
session_start();

require __DIR__ . '/db.php'; // must provide $pdo (PDO)
if ($pdo instanceof PDO) { $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); }

// PHPMailer + SMTP config (used by the public submit section below)
$autoload = __DIR__ . '/../vendor/autoload.php';
if (file_exists($autoload)) { require $autoload; }
require __DIR__ . '/mail.config.php';
// expects: SMTP_HOST, SMTP_USER, SMTP_PASS, SMTP_PORT, SMTP_FROM, SMTP_FROM_NAME, SMTP_TO, SMTP_TO_NAME
...
/* ---------------- flash (avoid undefined warnings) ---------------- */
$flash = [
  'ok'  => $_SESSION['flash_ok']  ?? [],
  'err' => $_SESSION['flash_err'] ?? [],
];
unset($_SESSION['flash_ok'], $_SESSION['flash_err']); // one-time display

/* ===================== A) PUBLIC SUBMIT HANDLER ===================== */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['public_submit'])) {
  ...
  try {
    ...
    // Admin notification (to your inbox only)
    // --- Sanitize sender fields ---
    ...
    // HTML body (formatted like your screenshot)
...
<div class="admin-shell">
  ...
  <main class="admin-main">
    ...
    <!-- ===== Contact Page Content (editable) ===== -->
    <div class="main-card">
      <h2 class="card-title"><i class="bi bi-pencil-square me-2"></i>Contact Page Content</h2>
    ...
    <div class="main-card">
      <h2 class="card-title"><i class="bi bi-inbox me-2"></i>Contact Inbox</h2>
