...
// adminProject.php
session_start();
require __DIR__ . '/db.php'; // must provide $pdo (PDO)
...
/** Save single upload into /uploads; returns '/uploads/xxx.ext' or null */
...
/** Convert '/uploads/foo.webp' to filesystem path for unlink */
...
/* ---- Create / Update project (duplicate guard + PRG) ---- */
if (($_POST['action'] ?? '') === 'save_project') {
  ...
  // duplicate guard: Title+Location (case-insensitive)
  try {
    if ($id > 0) {
  ...
  // cover upload (remember old cover to unlink if replaced)
  ...
  // NEW gallery images from main form (usually for create mode)
  $incomingIdxs  = [];
  ...
  /* ---- BACKEND NO-CHANGE GUARD (Edit mode) ---- */
  if (!$errors && $id > 0) {
    ...
    if ($cur) {
      ...
      if ($cover)                                           $changed = true; // new cover selected
      if (!empty($incomingIdxs))                            $changed = true; // new gallery files
    }
  ...
  if (!$errors) {
    try {
      if ($id > 0) {
        ...
        // Unlink old cover if we replaced it
      ...
      // Save gallery files (if any from this form)
...
/* ---- Replace a gallery image (single) ---- */
...
/* ---- Add more gallery images (multiple) ---- */
...
<head>
  ...
  <script>
    ...
    // Disable Update button until something changes (Edit mode only)
    document.addEventListener('DOMContentLoaded', function(){
      ...
      // serialize form state (excluding files and hidden action)
      const getState = () => {
        const data = {};
        ...
        // sort any arrays for stable compare
        for (const k in data) {
      ...
      if (isEdit) {
        ...
        setDisabled(false); // create mode: always enabled
      }
    });
  </script>
</head>
<body>
  <div class="admin-shell">
    ...
    <main class="admin-projects">
      ...
      <!-- ADD / EDIT FORM (standalone, no nested forms inside) -->
      <section class="card" style="margin-bottom:16px">
        ...
        <form id="project-form" method="post" enctype="multipart/form-data" class="grid-2">
          ...
          <?php if (!$editing): ?>
            <!-- Create mode: allow initial gallery -->
            <div class="grid" style="width: 95%;">
      ...
      <?php if ($editing): ?>
      <!-- GALLERY MANAGEMENT (separate card; separate forms) -->
      <section class="card" style="margin-bottom:16px">
